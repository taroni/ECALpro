    ...
    if ( not RunCRAB and not ONLYHADD and not ONLYFIT and not ONLYFINHADD ):
        ...
        for ijob in range(njobs):
            ...
            if ( mode.find('BATCH_RESU_SYST_1') != -1 ):
                 ...
                 SystParamLine = 'process.analyzerFillEpsilonForZ.SystOrNot = cms.untracked.double(1)\n'
                 env_script_n.write(SystParamLine)
                 env_script_n.close()
            if ( mode.find('BATCH_RESU_SYST_2') != -1 ):
                 env_script_n = open(outputdir + "/cfgFile/Fill/fillEpsilonPlot_iter_" + str(iters) + "_job_" + str(ijob) + ".py", 'a')
                 SystParamLine = 'process.analyzerFillEpsilonForZ.SystOrNot = cms.untracked.double(2)\n'
                 env_script_n.write(SystParamLine)
                 env_script_n.close()
            ...
            if not( '#FILE_APPEPENDED' in open(fit_cfg_n).read()):
               with open(fit_cfg_n, 'a') as file:
                    if( isOtherT2 and storageSite=="T2_BE_IIHE" and isCRAB ):
                        ...
                        file.write("process.fitEpsilonForZ.EpsilonPlotFileName = cms.untracked.string('dcap://maite.iihe.ac.be/pnfs/iihe/cms" + outLFN + "/iter_" + str(iters) + '/' + NameTag + "epsilonPlots.root')\n")
                        if(iters==0):
                           file.write("process.fitEpsilonForZ.calibMapPath = cms.untracked.string('dcap://maite.iihe.ac.be/pnfs/iihe/cms" + outLFN + "/iter_" + str(iters-1) + "/" + Add_path + "/" + NameTag + calibMapName + "')\n")
                        else:
                           file.write("process.fitEpsilonForZ.calibMapPath = cms.untracked.string('dcap://maite.iihe.ac.be/pnfs/iihe/cms" + outLFN + "/iter_" + str(iters-1) + "/" + Add_pathOLDIter + "/" + NameTag + calibMapName + "')\n")
                    else:
                        file.write("#FILE_APPEPENDED\n")
                        file.write("process.fitEpsilonForZ.EpsilonPlotFileName = cms.untracked.string('root://eoscms//eos/cms" + eosPath + "/" + dirname + "/iter_" + str(iters) + "/" + Add_path + "/" + NameTag + "epsilonPlots.root')\n")
                        if(iters==0):
                           file.write("process.fitEpsilonForZ.calibMapPath = cms.untracked.string('root://eoscms//eos/cms" + eosPath + "/" + dirname + "/iter_" + str(iters-1) + "/" + Add_path + "/" + NameTag + calibMapName + "')\n")
                        else:
                           file.write("process.fitEpsilonForZ.calibMapPath = cms.untracked.string('root://eoscms//eos/cms" + eosPath + "/" + dirname + "/iter_" + str(iters-1) + "/" + Add_pathOLDIter + "/" + NameTag + calibMapName + "')\n")
