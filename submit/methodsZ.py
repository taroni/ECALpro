from parametersZ import *

def printFillCfgZ1( outputfile, iteration ):
    ...
    # chiara: si chiamano EcalRecHitsEB ma sono reduced. Mentre il PF avrebbe bisogno dei non reduced 
    # che sono usati in RecoParticleFlow/PFClusterProducer/interface/PFEcalRecHitCreator.h

    #outputfile.write('process.load("RecoLocalCalo.EcalRecAlgos.EcalSeverityLevelESProducer_cfi")\n')   
    #outputfile.write('process.load("RecoLocalCalo.EcalRecProducers.ecalRecalibRecHit_cfi")\n') 
    #outputfile.write('process.ecalRecHit.doEnergyScale = cms.bool(' + doEnenerScale + '),\n')        
    #outputfile.write('process.ecalRecHit.doIntercalib = cms.bool(' + doIC + '),\n')                  
    #outputfile.write('process.ecalRecHit.doLaserCorrections = cms.bool(' + doLaserCorr + '),\n')     
    #outputfile.write('process.ecalRecHit.EBRecHitCollection = cms.string("reducedEcalRecHitsEB"),\n')
    #outputfile.write('process.ecalRecHit.EERecHitCollection = cms.string("reducedEcalRecHitsEE"),\n')
    #outputfile.write('process.ecalRecHit.EBRecalibRecHitCollection = cms.string("EcalRecHitsEB"),\n')
    #outputfile.write('process.ecalRecHit.EERecalibRecHitCollection = cms.string("EcalRecHitsEE")\n')
    #outputfile.write('    )\n')

    outputfile.write('process.load("CalibCode.FillEpsilonPlot.calibRechitsForZProducer_cfi")\n')
    outputfile.write("process.ecalRecHit.CurrentIteration = cms.untracked.int32(" + str(iteration) + ")\n")
    if (isCRAB):
        outputfile.write("process.ecalRecHit.calibMapPath = cms.untracked.string('CalibCode/FillEpsilonPlot/data/" + NameTag + calibMapName + "')\n")
        outputfile.write("process.ecalRecHit.isCRAB  = cms.untracked.bool(True)\n")
    else:
        if (SubmitFurtherIterationsFromExisting and iteration == 0):
            outputfile.write("process.ecalRecHit.calibMapPath = cms.untracked.string('root://eoscms//eos/cms" + startingCalibMap + "')\n")
        else:
            outputfile.write("process.ecalRecHit.calibMapPath = cms.untracked.string('root://eoscms//eos/cms" + eosPath + "/" + dirname + "/iter_" + str(iteration-1) + "/" + NameTag + calibMapName + "')\n")

    outputfile.write("process.ecalRecHit.Barrel_orEndcap = cms.untracked.string('" + Barrel_or_Endcap + "')\n")
    outputfile.write('\n')

    # chiara: non so come cambiare il nome alla collezione di rechits reduced che deve diventare EcalRecHitsES
    # invece di reducedEcalRecHitsES => questo modulo dummy si limita a cambiare il nome
    outputfile.write('### Dummy module to change the collection name\n')
    outputfile.write('process.load("CalibCode.FillEpsilonPlot.esDummyRecHit_cfi")\n')   
    outputfile.write('process.ecalPreshowerRecHit.ESRecHitCollection = "reducedEcalRecHitsES"\n')
    outputfile.write('\n')
...
def printFillCfgZ2( outputfile, pwd , iteration, outputDir, ijob ):
    outputfile.write("    )\n")
    outputfile.write(")\n")
