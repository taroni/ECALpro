...
print "[calib] Storing parametersZ.py for future reference"
CopyParam = subprocess.Popen(['cp  parametersZ.py ' + workdir], stdout=subprocess.PIPE, shell=True);
...
for iter in range(nIterations):
    ...
    while (len(inputlist_v) > 0):
        ...
        printFillCfgZ1( fill_cfg_f, iter )
        ...
        if( isCRAB ):
            printFillCfgZ2( fill_cfg_f, pwd, iter , "", ijob )
        else: 
            printFillCfgZ2( fill_cfg_f, pwd, iter , "/tmp/", ijob )
        fill_cfg_f.close()
...
for iter in range(nIterations):
    ...
    for nFit in range(nEB):
        ...
        # print the cfg file
        printFitCfgZ( fit_cfg_f , iter, "/tmp",inListB[nFit],finListB[nFit],"Barrel",nFit)
        fit_cfg_f.close()
    ...
    for nFit in range(nEE):
        ...
        # print the cfg file
        printFitCfgZ( fit_cfg_f , iter, "/tmp",inListE[nFit],finListE[nFit],"Endcap",nFit)
...
env_script_f.write( "python " + pwd + "/calibJobHandlerZ.py " + str(njobs) + " " + queue + "\n")
